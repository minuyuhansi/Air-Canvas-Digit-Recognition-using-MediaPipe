...
# Global variable to receive results from the background thread
latest_result = None

# Kept free of strict type hints to prevent Pylance squiggles
def print_result(result, output_image, timestamp_ms):
    global latest_result
    latest_result = result

# Configure options for LIVE_STREAM mode
...
# Load the 0-9 Digit Recognition Model using OpenCV DNN
try:
...
# Everything using 'recognizer' stays inside this 'with' block context
with GestureRecognizer.create_from_options(options) as recognizer:
    ...
    while webCam.isOpened():
        ...
        frame_counter += 1
        # Now 'recognizer' is perfectly defined because we are inside the block!
        ...
        if latest_result is not None:
            ...
            cv2.putText(frame, f"Gesture: {gesture_name}", (20, 40), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)
            cv2.putText(frame, f"Predicted Digit: {detected_number}", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2, cv2.LINE_AA)
        ...
        for i in range(1, len(drawing_points)):
            if drawing_points[i - 1] is None or drawing_points[i] is None: 
                continue
            cv2.line(frame, drawing_points[i - 1], drawing_points[i], (0, 0, 255), 6)
